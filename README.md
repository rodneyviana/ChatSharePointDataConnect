...
2. **Set up the database** (choose one method):
   ...
   **Option A: Restore from backup** (includes sample data - READY TO USE)

   ? **Recommended for quick start** - Database is already populated with sample SharePoint data
   ...
   ? **Skip to step 4** - Database is ready, no need to run SPGraphConnector project
...
3. **Set up AI provider** (Ollama recommended):
   ```bash
...
**Purpose:** Import SharePoint data from Microsoft Graph Data Connect into a SQL Server database
...
?? **[View detailed documentation ?](SPGraphConnector/README.md)**
...
cd securitybot
cp .env.example .env
# Configure .env with Ollama or Azure OpenAI settings
dotnet run
```

?? **[View detailed documentation ?](securitybot/README.md)**

## ?? When to Use Each Project

### Use SecurityBot (Project 2) when:
? You restored the database from .bak or .bacpac backup  
? You want to query SharePoint data with natural language  
? You're ready to test the AI chatbot  

**? Start here after restoring the database backup!**

### Use SPGraphConnector (Project 1) when:
? You have your own Graph Data Connect exports to import  
? You created an empty database and want to use the included sample files  
? You want to refresh the database with new data  
...
- **Purpose:** ? **Fastest way to get started** - restore and immediately use SecurityBot!
...
? **After restore:** Database is ready! Skip to SecurityBot configuration (no need to run SPGraphConnector)
...
? **After import:** Database is ready! Skip to SecurityBot configuration (no need to run SPGraphConnector)
...
- **Data Privacy:** 
  ...
  - Azure OpenAI sends queries to Microsoft's cloud
...
**Built with ?? using .NET 8, Semantic Kernel, and Ollama**
